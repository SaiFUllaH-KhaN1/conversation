...
const InterbotChat = ({modeStr, exitValue, textTextBlock, interBotPersona}) => {
    ...
    const confirmSubmission = (e) => {
        ...
        const idPvalue = document.getElementById("idP").textContent;
        ...
        const responseFunctionCall = async () => {
            const responseVar = await fetch("http://127.0.0.1:6770/converse",
            {
                ...
                body: JSON.stringify({
                    ...
                    interBotPersona: interBotPersona,
                  })
            });
            ...
            reader.read().then(function processResult(result) {
                if (result.done) {
                    ...
                    // document.getElementById('submissionBut').style.visibility = "visible";
                    console.log("Done");
    ...
    const idPHistoryFunc = () => {
        ...
        y.forEach((ele, idx) => {
            if (idx === 0) {
                nodes.push(<div className='botHistoryInstance' key={`bot-${idx}`}> <p id='botHistoryInstance'>{ele.Bot}</p> </div>);
            } else {
                nodes.push(<div key={`human-${idx}`}> <div className='humanHistoryInstance'><p id='humanHistoryInstance'>{ele.Human}</p></div> <div className='botHistoryInstance'><p id='botHistoryInstance'>{ele.Bot}</p></div> </div>);
            }
        });
    ...
    useEffect(()=>
    {
        if (oneTime.current===false){
            ...
            document.getElementById('submissionBut').style.visibility = "hidden";
            chatVarSetterFunction('');
        ...
        <div className='main-chat'>
            ...
            <div className='exitButtonDivAndModeDiv'>
                <div className="modeDiv">
                    <p>Mode: Inter Bot Conversation</p>
                </div>
            ...
            <div className="chat-controls">
                ...
                <button id="submissionBut" onClick={confirmSubmission}>&#9654;</button>
            </div>
